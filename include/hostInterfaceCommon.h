...
// Get sleep mode
#define RCM_GET_GPIO_CONFIG_REQUEST		(RCRM_MSG_TYPE_REQUEST + 0x12)
#define RCM_GET_GPIO_CONFIG_CONFIRM		(RCRM_MSG_TYPE_CONFIRM + 0x12)

// Get sleep mode
#define RCM_SET_GPIO_CONFIG_REQUEST		(RCRM_MSG_TYPE_REQUEST + 0x13)
#define RCM_SET_GPIO_CONFIG_CONFIRM 	(RCRM_MSG_TYPE_CONFIRM + 0x13)

// Get sleep mode
#define RCM_GET_GPIO_REQUEST 			(RCRM_MSG_TYPE_REQUEST + 0x14)
#define RCM_GET_GPIO_CONFIRM 			(RCRM_MSG_TYPE_CONFIRM + 0x14)

// Get sleep mode
#define RCM_SET_GPIO_REQUEST 			(RCRM_MSG_TYPE_REQUEST + 0x15)
#define RCM_SET_GPIO_CONFIRM 			(RCRM_MSG_TYPE_CONFIRM + 0x15)
...
// gpio command structures


typedef struct
{
	// set to RCM_GET_GPIO_CONFIG_REQUEST (0xF012)
	mrm_uint16_t msgType;
	// identifier to correlate requests with info messages
	mrm_uint16_t msgId;
} mrmMsg_GetGpioConfigRequest ;



typedef struct
{
	// set to RCM_GET_GPIO_CONFIG_CONFIRM (0xF112)
	mrm_uint16_t msgType;
	// identifier to correlate requests with info messages
	mrm_uint16_t msgId;
	// Specifies the mode for each GPIO, 2 bits per GPIO.
	mrm_uint32_t gpioMode;
	// GPIO Direction
	mrm_uint16_t gpioDirection;
	// Reserved
	mrm_uint16_t reserved;

} mrmMsg_GetGpioConfigConfirm ;



typedef struct
{
	// set to RCM_SET_GPIO_CONFIG_REQUEST (0xF013)
	mrm_uint16_t msgType;
	// identifier to correlate requests with info messages
	mrm_uint16_t msgId;
	// Specifies the mode for each GPIO, 2 bits per GPIO.
	...
	// Reserved
	mrm_uint8_t reserved;
	// Specifies how this configuration record will persist through power cycling (write to flash memory).
	mrm_uint8_t persistFlag;

} mrmMsg_SetGpioConfigRequest ;



typedef struct
{
	// set to RCM_SET_GPIO_CONFIG_CONFIRM (0xF113)
	mrm_uint16_t msgType;
	// identifier to correlate requests with info messages
	mrm_uint16_t msgId;
	// status
	mrm_uint32_t status;
} mrmMsg_SetGpioConfigConfirm ;



typedef struct
{
	// set to RCM_GET_GPIO_ REQUEST (0xF014)
	mrm_uint16_t msgType;
	// identifier to correlate requests with info messages
	mrm_uint16_t msgId;
} mrmMsg_GetGpioRequest ;


typedef struct
{
	// set to RCM_GET_GPIO_CONFIRM (0xF114)
	mrm_uint16_t msgType;
	// identifier to correlate requests with info messages
	mrm_uint16_t msgId;
	// Specifies the state for each GPIO. Bit 0 is mapped to GPIO 0
	mrm_uint16_t gpioState;
	// Specifies the state of each GPIO set to I/O as an output. Used to verify user settings
	mrm_uint16_t gpioValue;
} mrmMsg_GetGpioConfirm ;


typedef struct
{
	// set to RCM_SET_GPIO_REQUEST (0xF015)
	mrm_uint16_t msgType;
	// identifier to correlate requests with info messages
	mrm_uint16_t msgId;
	// Drives a 0 or 1 onto a GPIO configured as output. Ignored for others. Bit 0 is mapped to GPIO 0
	mrm_uint16_t gpio;
	// Masks the bits in GPIO so that only GPIOs set in the Mask are changed
	mrm_uint16_t mask;
	// reserved1
	mrm_uint16_t reserved1;
	// reserved2
	mrm_uint8_t reserved2;
	// Specifies how this configuration record will persist through power cycling (write to flash memory).
	mrm_uint8_t persistFlag;
} mrmMsg_SetGpioRequest ;


typedef struct
{
	// set to RCM_SET_GPIO_CONFIRM (0xF115)
	mrm_uint16_t msgType;
	// identifier to correlate requests with info messages
	mrm_uint16_t msgId;
	// status
	mrm_uint32_t status;
} mrmMsg_SetGpioConfirm ;
