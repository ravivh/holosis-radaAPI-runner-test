...
int nxpIMX6DQRMPinInterruptMask( int i, uint8_t mode ) {
	struct layout_t *pin = NULL;
//	unsigned long addrSel = 0;
	unsigned long addrDat = 0;
	uint32_t val = 0;
	...
	pin = &nxpIMX6DQRM->layout[nxpIMX6DQRM->map[i]];
//	addrSel = (unsigned long)(nxpIMX6DQRM->gpio[pin->addr] + nxpIMX6DQRM->base_offs[pin->addr] + pin->select.offset);
	addrDat = (unsigned long)(nxpIMX6DQRM->gpio[pin->addr] + nxpIMX6DQRM->base_offs[pin->addr] + pin->data.offset + 14);
	pin->mode = mode;
...
int nxpIMX6DQRMGetPinMux(int i)
{
#if 0
	struct layout_t *pin = NULL;
	unsigned long addrSel = 0;
...
//	mux = soc_readl(addrSel);
//	printf("sel %08lX mux data %08X \n", addrSel , mux );
//	dir = soc_readl(addrDat);
//	printf("direction is %X" , dir & (1 << pin->data.bit) );
//	val = soc_readl(addrDat-4);
//	printf("val is %X" , val & (1 << pin->data.bit) );
#endif
	return 0;
}
