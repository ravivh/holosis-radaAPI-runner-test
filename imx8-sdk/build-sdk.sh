...
: ${DEBIAN_PACKAGES:="apt-transport-https,busybox,ca-certificates,can-utils,chrony,curl,e2fsprogs,ethtool,fdisk,gpiod,haveged,i2c-tools,ifupdown,iputils-ping,isc-dhcp-client,initramfs-tools,libiio-utils,lm-sensors,locales,nano,net-tools,ntpdate,openssh-server,psmisc,rfkill,sudo,systemd,systemd-sysv,dbus,tio,usbutils,wget,xterm,xz-utils,gcc,cloud-guest-utils,unzip"}
: ${HOST_NAME:=imx8mp}
...
set -e
echo "Completed Bootloader build."
}
...
build_linux() {
    ...
    echo "Completed Linux Kernel build."
}
...
build_rootfs() {
    echo "Building Root Filesystem (${DISTRO})..."
    ...
    echo "Completed Root Filesystem build."
}
...
assemble_boot_image() {
    ...
    echo "Boot Image assembly complete."
}
...
show_help() {
    ...
    echo "  --optee        Build only OPTEE."
    echo "  --bootloader   Build only Bootloader."
    echo "  --linux        Build only Linux Kernel."
    echo "  --rootfs       Build only Root Filesystem."
    echo "  --image        Assemble the Boot Image."
    echo "  --help         Show this help message."
}

if [[ $# -eq 0 ]]; then
    echo "No options provided. Building all components..."
    do_build_opteeos
    build_bootloader
    build_linux
    build_rootfs
